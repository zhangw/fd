...
pub fn ensure_search_pattern_is_not_a_path(opts: &Opts) -> Result<()> {
    if !opts.full_path
...
pub fn build_pattern_regex(pattern: &str, opts: &Opts) -> Result<String> {
...
pub fn check_path_separator_length(path_separator: Option<&str>) -> Result<()> {
...
pub fn construct_config(mut opts: Opts, pattern_regexps: &[String]) -> Result<Config> {
...
pub fn extract_command(opts: &mut Opts, colored_output: bool) -> Result<Option<CommandSet>> {
    opts.exec
        .command
        .take()
        .map(Ok)
...
pub fn determine_ls_command(colored_output: bool) -> Result<Vec<&'static str>> {
    #[allow(unused)]
...
pub fn extract_time_constraints(opts: &Opts) -> Result<Vec<TimeFilter>> {
...
pub fn ensure_use_hidden_option_for_leading_dot_pattern(
    config: &Config,
...
pub fn build_regex(pattern_regex: String, config: &Config) -> Result<regex::bytes::Regex> {
...
pub fn set_working_dir(opts: &Opts) -> Result<()> {
